...
const TabsContainer = styled.div`
    justify-content: ${props => props.slot == "left" ? "flex-start" : props.slot == "right" ? "flex-end" : "flex-start"};
    height: min-content;
    width: 100%;
    display: flex;
    flex-direction: row;
...
const Tab = styled.div`
    ...
    margin-left: ${props => props.slot == "left" ? "0px" : props.slot == "right" ? "20px" : "0px"};
    margin-right: ${props => props.slot == "left" ? "20px" : props.slot == "right" ? "0px" : "20px"};
    font-weight: bold;
    cursor: pointer;
    :hover {
        opacity: .6;
...
function TabbedGallery(props) {
    const [selected, setSelected] = useState(props.default || Object.keys(props.tabs)[0]);

    function getTabs(){
        let tabsList = []
        for (const key of Object.keys(props.tabs)){
            tabsList = [...tabsList, <Tab selected={key == selected} onClick={() => setSelected(key)} slot={props.slot}>{key}</Tab>]
        }
        return tabsList;
    }
    ...
    function getImages(){
        ...
        for (const src of props.tabs[selected]){
            imagesList = [...imagesList, <ImgContainer><Img src={src}/></ImgContainer>]
        }
        return imagesList;
    }
